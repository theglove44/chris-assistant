...
| 3 | 🔴 | ✅ | **Increase tool turn limit** | All three providers now use `config.maxToolTurns` (default 200, configurable via `MAX_TOOL_TURNS` env var). Set high because context compaction (`providers/compaction.ts`) keeps conversations within the model's window. Claude's `maxTurns`, OpenAI and MiniMax loop limits all read from the same config. |
...
| 3 | 🟡 | ✅ | **Heartbeat file** | `src/heartbeat.ts` — writes `HEARTBEAT.md` to the root of the GitHub memory repo every 3 hours (+ immediately on startup). Collects: uptime, started-at timestamp, current model/provider, health status for GitHub repo + MiniMax/OpenAI tokens (with warning thresholds matching `health.ts`), all scheduled tasks with cron expressions and last-run times, last message relative time, and today's message count from the JSONL archive. SHA-256 dedup skips unchanged writes. Integrated into `index.ts` lifecycle (`startHeartbeat`/`stopHeartbeat`). |
