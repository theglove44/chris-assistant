...
| # | Impact | Status | Item | Description |
|---|--------|--------|------|-------------|
| 1 | 🔴 | ⬜ | **Daily memory journal** | The bot should write structured notes throughout the day as things happen — not just a single AI summary at 23:55. OpenClaw writes entries like "Phil email check ran at 2:15pm — picked up Thu 20 Feb briefing" as events occur. New `src/memory/journal.ts` module. The bot calls a `journal_entry` tool (or an internal function triggered after significant interactions) to append timestamped notes to today's journal in the memory repo (`journal/YYYY-MM-DD.md`). Categories: decisions made, topics discussed, tasks completed, things learned, mood/tone observations. The daily summary at 23:55 becomes a *consolidation* of the journal rather than a cold summary of raw messages. Journal entries are richer than JSONL archives because they capture the bot's interpretation, not just the raw text. |
| 2 | 🟠 | ⬜ | **Curated MEMORY.md** | Single long-term memory document that the bot actively maintains — a living dossier of everything it knows about Chris, organized by topic. Inspired by OpenClaw's massive `MEMORY.md` which covers who Chris is, trading history, projects, financial context, communication patterns, preferences, and key insights. Currently we have split files (about-chris.md, preferences.md, projects.md, people.md) which are fine but fragmented. New approach: add a `memory/SUMMARY.md` file that gets periodically consolidated from (a) the split knowledge files, (b) recent daily journals, and (c) the bot's own observations. A weekly consolidation task (Sunday night) reads the past week's journals and updates SUMMARY.md — merging new facts, removing stale info, and maintaining a coherent narrative. The split files remain as the source of truth for `update_memory`, but SUMMARY.md becomes the primary context the bot reads. Cap at ~8K tokens to keep system prompt manageable. |
| 3 | 🟡 | ⬜ | **Heartbeat file** | A living `HEARTBEAT.md` in the memory repo that the bot updates periodically (every few hours) with its current status: what's working, what's broken, recent activity summary, upcoming scheduled tasks, and any concerns. OpenClaw maintains one that tracks cron job health, model status, and system state. Useful for: (a) the bot itself to read on startup and quickly orient, (b) Chris to glance at the bot's self-reported status, (c) debugging — a persistent log of the bot's self-awareness. Written by a lightweight heartbeat function that runs on the existing health check interval (every 5 min update, write to GitHub every 3 hours). |
| 4 | 🟡 | ⬜ | **Memory consolidation loop** | Automated weekly process that reads recent daily journals and summaries, identifies new persistent facts (about Chris, his projects, preferences, people), and proposes updates to the knowledge files. Similar to how OpenClaw's MEMORY.md gets curated over time. Could run as a built-in scheduled task (like the daily summarizer) — Sunday at 23:00. Calls `chat()` with a consolidation prompt that includes the week's journals + current knowledge files, outputs structured `update_memory` calls. Prevents memory staleness — projects that Chris stopped working on get noted, new people get added, preferences evolve. |
