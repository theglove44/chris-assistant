...
│   ├── conversation-channel-summary.ts # Weekly per-channel summarizer — Sunday 23:50, per Discord channel
│   ├── memory-consolidation.ts # Weekly memory consolidation — curates SUMMARY.md from all sources
│   ├── dashboard.ts          # Built-in web dashboard — HTTP server, JSON API, inline SPA (Status/Schedules/Conversations/Memory/Logs)
...
- **Web dashboard**: `src/dashboard.ts` — built-in HTTP server using Node's `http` module (zero new dependencies). Serves a single HTML page with all CSS/JS inlined as template strings. 5 tabs: Status & Health (uptime, model, pm2 stats, health check indicators), Schedules (cron jobs with config/lastRun/tools), Conversations (browse archives by date, daily summaries), Memory (view/edit GitHub memory files), Logs (real-time SSE tail of pm2 logs via `fs.watch`). JSON API endpoints at `/api/*` power the frontend. Auth: if `DASHBOARD_TOKEN` env var is set, requires token via query param or Bearer header (enables Tailnet access); otherwise localhost-only. Port configurable via `DASHBOARD_PORT` (default 3000). `startDashboard()`/`stopDashboard()` wired into `index.ts` lifecycle. pm2 stats cached 30s via `getBotProcess()` from `pm2-helper.ts`. Health data exposed via `getHealthStatus()` exported from `health.ts` (caches last check results). Gracefully handles port-in-use (logs warning, disables dashboard). **UI components**: toast notifications (`showToast(msg, type)` — success/error/info, auto-dismiss 3s, bottom-right), designed empty states (icon + heading + description for each tab's zero-data view), skeleton loaders (shimmer animation replacing all "Loading..." text), top-of-page progress bar (reference-counted across concurrent API calls), CSS-only tooltips (`[data-tooltip]` on schedule prompts, cron descriptions, health details), inline alert banners above health dots when providers fail, segmented control (`Live | Snapshot` in Logs tab replacing checkbox), unified `.badge-pill` class system (success/error/neutral/accent variants), and a right-side drawer for the schedule editor (replaces centered modal, 480px slide-in with `drawerCloseTimer` to prevent race conditions on rapid open/close).
...
- **Memory storage**: Markdown files in a private GitHub repo. Every update is a git commit — fully auditable and rollback-able. Octokit logging is silenced (`log: { debug/info/warn/error: () => {} }`) to prevent expected 404s (e.g. missing daily summaries) from polluting pm2 error logs.
- **Persistent conversation history**: Last 20 messages per chat stored in `~/.chris-assistant/conversations.json`. Loaded lazily on first access, saved asynchronously via a write queue that serializes concurrent saves. Callers use fire-and-forget for `addMessage()` and await `clearHistory()`. `addMessage()` accepts optional metadata (`{ source?, channelName? }`) which is passed through to `archiveMessage()`. Survives restarts. `/clear` wipes conversation window + Claude session. `/purge` additionally redacts today's archive.
...
- **Conversation archive**: `conversation-archive.ts` appends every message (user + assistant) as a JSONL line to `~/.chris-assistant/archive/YYYY-MM-DD.jsonl` via synchronous `appendFileSync` (microseconds, never throws). Called from `addMessage()` in `conversation.ts` before the rolling window trims old messages. Each entry includes optional `source` (`"telegram"` or `"discord"`) and `channelName` fields for filtering by platform and channel. A periodic uploader (every 30 minutes) pushes changed archive files to `archive/YYYY-MM-DD.jsonl` in the memory repo using SHA-256 dedup. On graceful shutdown, `uploadArchives()` is called to flush pending archives before the process exits. Exports `readLocalArchive(date)`, `listLocalArchiveDates()`, `datestamp()`, `localArchivePath()`, `uploadArchives()`, and `redactArchiveEntries()` for use by the recall tool, summarizer, and `/purge` command.
...
- **Weekly channel summaries**: `src/conversation-channel-summary.ts` — built-in module that fires Sunday at 23:50 local time. Reads the past 7 days of local archives, groups entries by `channelName` field, and generates a focused AI summary for each Discord channel that had activity. Summaries are written to `conversations/channels/<sanitized-name>/YYYY-WXX.md` in the memory repo. Channel names are sanitized (lowercase, hyphens, alphanumeric only). Uses ISO week numbering with double-fire prevention. `startChannelSummarizer()`/`stopChannelSummarizer()` wired into `index.ts` lifecycle.
- **Conversation purge**: `/purge` Telegram command for full privacy clear. Clears the rolling conversation window, resets the Claude session, and redacts today's archive entries for the chat via `redactArchiveEntries()`. Writes an empty file (not delete) when all entries are removed so `uploadArchives()` overwrites the GitHub copy. Triggers an immediate archive upload after redaction. Added to the Telegram command menu and `/help` output.
...
- **Market snapshot tool**: `src/tools/market-snapshot.ts` — `market_snapshot` tool (category `"always"`) that SSHes to the Mac Mini (`100.99.188.80`) to run `tasty-coach --snapshot`. Parses the raw table output into structured `SnapshotItem` objects (symbol, last, change, direction). Formats for Telegram with HTML bold headers — categorized sections for Equities (/ES, /NQ, /YM, /RTY), Commodities (Gold, Silver, Crude), Crypto (BTC), and Volatility (VIX). Auto-generates trading insights based on market conditions (broad green/red, biggest movers, VIX spikes, safe haven flows). Uses the same SSH constants as `ssh.ts` (`BatchMode=yes`, 60s timeout). Designed for scheduled daily market reports (8am and 2pm).
...
- **Telegram command menu**: Bot registers `/start`, `/clear`, `/purge`, `/stop`, `/session`, `/model`, `/memory`, `/project`, `/reload`, `/restart`, `/help` via `setMyCommands` on startup. Commands appear in Telegram's bot menu. `/model` shows current model/provider. `/memory` lists all memory files with sizes from GitHub. `/reload` invalidates the system prompt cache so the next message reloads memory from GitHub. `/stop` aborts the current Claude query via AbortController. `/session` shows the active Claude session ID. `/clear` resets both conversation history and Claude session.
...
- **Dashboard inline JS string escaping**: The dashboard (`dashboard.ts`) serves all JS inline in a template literal. Backslash escapes like `\'` in the TypeScript source are consumed by the template literal itself — they don't appear in the browser output. This means `'onclick="fn(\'' + id + '\')"'` produces broken JS (`fn('' + id + '')`). Instead, use `data-*` attributes with `addEventListener` for click handlers that need dynamic values: `'<tr data-id="' + id + '">'` then `row.addEventListener("click", function() { fn(row.dataset.id); })`. Same principle applies to any quoted strings inside inline event handlers.
- **Dashboard UI patterns**: All UI components are vanilla CSS/JS with no dependencies. Use `var` (not `const`/`let`) for inline JS consistency. New CSS goes after `.progress-bar.done` and before `.flex-between` in the style block. Animations use double `requestAnimationFrame` for enter transitions (ensures browser paints initial state before triggering). The drawer's `closeScheduleModal()` stores its hide-timeout in `drawerCloseTimer` so `openScheduleModal()` can cancel it — without this, rapid close-then-open causes the stale timer to hide the drawer mid-open. The progress bar uses a `progressCount` reference counter so overlapping API calls keep it active until all complete. For `data-tooltip` attributes, escape `"` as `&quot;` via `.replace(/"/g, '&quot;')` after `esc()`.
